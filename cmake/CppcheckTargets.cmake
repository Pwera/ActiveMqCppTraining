...
function(add_cppcheck _name)
    ...
    if (CPPCHECK_FOUND)
        ...
        if ("${_unused_func}" GREATER "-1")
            message(INFO " For ${_name} CppCheck assign [UNUSED_FUNCTIONS] ${CPPCHECK_UNUSEDFUNC_ARG}" )
            list(APPEND _cppcheck_args ${CPPCHECK_UNUSEDFUNC_ARG})
        endif ()

        list(FIND ARGN STYLE _style)
        if ("${_style}" GREATER "-1")
            message(INFO " For ${_name} CppCheck assign [STYLE] ${CPPCHECK_STYLE_ARG}")
            list(APPEND _cppcheck_args ${CPPCHECK_STYLE_ARG})
        endif ()

        list(FIND ARGN POSSIBLE_ERROR _poss_err)
        if ("${_poss_err}" GREATER "-1")
            message(INFO " For ${_name} CppCheck assign [POSSIBLE_ERROR] ${CPPCHECK_POSSIBLEERROR_ARG}")
            list(APPEND _cppcheck_args ${CPPCHECK_POSSIBLEERROR_ARG})
        endif ()

        list(FIND ARGN FORCE _force)
        if ("${_force}" GREATER "-1")
            message(INFO " For ${_name} CppCheck assign [FORCE]")
            list(APPEND _cppcheck_args "--force")
        endif ()

        list(FIND ARGN PERFORMANCE _force)
        if ("${_force}" GREATER "-1")
            message(INFO " For ${_name} CppCheck assign [PERFORMANCE]")
            list(APPEND _cppcheck_args "--enable=performance")
        endif ()
        ...
        if ("${_fail_on_warn}" GREATER "-1")
            list(APPEND
                    ...
                    ${CPPCHECK_WARN_REGULAR_EXPRESSION})
            list(REMOVE_AT _input ${_unused_func})
            message(INFO " For ${_name} CppCheck assign [FAIL_ON_WARNINGS]")
        endif ()

        get_target_property(_cppcheck_includes "${_name}" INCLUDE_DIRECTORIES)
        message(INFO " For ${_name} CppCheck _cppcheck_includes ${_cppcheck_includes}")
        set(_includes)
        ...
        if ("1.${CMAKE_VERSION}" VERSION_LESS "1.2.8.0")
            ...
            add_test(NAME
                    ...
                    ${_cppcheck_args}
                    ${_files})
            list(LENGTH _files size)
            message(INFO " For ${_name} CppCheck execute CPPCHECK_TEMPLATE_ARG ${CPPCHECK_TEMPLATE_ARG} _cppcheck_args ${_cppcheck_args} ${size}")
        endif ()
